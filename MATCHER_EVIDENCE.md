...
| Measurement integrity | [`tests/test_discriminative.py`](tests/test_discriminative.py) fails locally if any system saturates the benchmark or if a ground-truth lookup can pass as grounded retrieval | Guards this benchmark, not benchmarks in general |
...
| Reproducibility | Every input content-hashed in [`data/manifest.json`](data/manifest.json) and verified before the run aborts or proceeds; offline Docker; local tests | Single-process demo, no cloud infrastructure |
